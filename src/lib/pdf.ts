...
export function generateMeetingReportPdf(reportText: string, meetingTitle: string): void {
  const pdf = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  const LEFT = 25
  const RIGHT = 185
  const USABLE = RIGHT - LEFT
  ...
  const HEADER_KEYS = /^(MEETING|DATE|TYPE|PARTNERSHIP):/
  const SECTION_HEADINGS = new Set(['ATTENDEES', 'AGENDA', 'MINUTES', 'ACTION POINTS'])
  let pastTitle = false
  let pastHeader = false
  ...
  for (const rawLine of reportText.split('\n')) {
    ...
    if (line === 'MEETING REPORT') {
      pdf.setFontSize(17)
      pdf.setFont('helvetica', 'bold')
      pdf.setTextColor(30, 30, 30)
      pdf.text('MEETING REPORT', 210 / 2, y, { align: 'center' })
      const tw = pdf.getTextWidth('MEETING REPORT')
      pdf.setDrawColor(232, 98, 26)
      pdf.setLineWidth(0.7)
      pdf.line(210 / 2 - tw / 2, y + 1.8, 210 / 2 + tw / 2, y + 1.8)
      y += LH * 2
    ...
    } else if (pastTitle && HEADER_KEYS.test(line)) {
      const colonIdx = line.indexOf(':')
      const key = line.substring(0, colonIdx + 1)
      const val = line.substring(colonIdx + 2)
      pdf.setFontSize(10)
      ...
      const wrapped = pdf.splitTextToSize(val, USABLE - 32) as string[]
      pdf.text(wrapped[0] ?? '', LEFT + 32, y)
      for (let i = 1; i < wrapped.length; i++) { y += LH; pdf.text(wrapped[i], LEFT + 32, y) }
      y += LH

    } else if (pastTitle && !pastHeader && line === '') {
      // divider after last header key (empty line following header block)
      pdf.setDrawColor(210, 210, 210)
      pdf.setLineWidth(0.3)
      pdf.line(LEFT, y + 1, RIGHT, y + 1)
      y += LH
  ...
  const dateStamp = new Date().toLocaleDateString('en-GB').replace(/\//g, '-')
  pdf.save(`${meetingTitle} — Report — ${dateStamp}.pdf`)
}
